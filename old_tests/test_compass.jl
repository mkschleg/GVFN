...
function make_features_2!(ϕ, obs_env, obs, action, preds)
    fill!(ϕ, 0.0)
    place = (action - 1)*(8*5 + 1 + 5) + 1
    ϕ[place] = 1

    
    place += 1
    ϕ[place:(place+5)] = obs_env

    place += 6
    ϕ[place:(place+8*5-1)] = obs
...
function run_gvfn(numsteps; α=0.1, α_2=0.1, λ=0.9, pertubation=nothing, ORACLE=nothing, gvfn=Networks.CycleWorld.GammaChain, bptt=false)
    ...
    gvfn_2 = Networks.CompassWorld.Gamma

    gvfn_obj = gvfn.build_gvfn()

    gvfn_obj_2 = gvfn_2.build_gvfn()
    ...
    numgvfs_2 = gvfn_2.weight_dim[1]
    numweights_2 = gvfn_2.weight_dim[2]
