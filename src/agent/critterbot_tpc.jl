export CritterbotTPCGVFNAgent, CritterbotTPCRNNAgent, CritterbotTPCAuxTaskAgent,
    CritterbotTPCOriginalRNNAgent, CritterbotTPCOriginalAuxTaskAgent,
    predict!
...
mutable struct CritterbotTPCAgent{L, O, C, N, H, Φ, F} <: MinimalRLCore.AbstractAgent where {L<:LearningUpdate}
    lu::L
    opt::O
    chain::C
    normalizer::N
...
function CritterbotTPCGVFNAgent(parsed; rng=Random.GLOBAL_RNG)
...
function CritterbotTPCOriginalRNNAgent(parsed; rng=Random.GLOBAL_RNG)
    ...
    return _CritterbotTPCRNNAgent(parsed, chain, normalizer; rng=rng)
end

function CritterbotTPCRNNAgent(parsed; rng=Random.GLOBAL_RNG)
...
function CritterbotTPCTCAgent(parsed; rng=Random.GLOBAL_RNG)
...
function CritterbotTPCFCAgent(parsed; rng=Random.GLOBAL_RNG)
...
function _CritterbotTPCRNNAgent(parsed, chain, normalizer; rng=Random.GLOBAL_RNG)
...
function resetBatch!(agent::CritterbotTPCAgent)
...
function MinimalRLCore.start!(agent::CritterbotTPCAgent, env_s_tp1, rng=Random.GLOBAL_RNG)
...
function MinimalRLCore.step!(agent::CritterbotTPCAgent, env_s_tp1, r, terminal, rng=Random.GLOBAL_RNG)
    ...
    if DataStructures.isfull(agent.obs_buff)
        ...
        reset!(agent.chain, agent.hidden_state_init)
        v_tp1 = Flux.data(agent.chain.(agent.obs_sequence)[end])
        push!(agent.batch_model_target, r[1] .+ r[2]*v_tp1)
...
function predict!(agent::CritterbotTPCAgent, env_s_tp1, r, terminal, rng=Random.GLOBAL_RNG)
...
mutable struct CritterbotTPCAuxTaskAgent{L, G, O, C, N, H, Φ} <: MinimalRLCore.AbstractAgent where {L<:LearningUpdate, G<:AbstractHorde}
    lu::L
    horde::G
    opt::O
    chain::C
...
num_gvfs(a::CritterbotTPCAuxTaskAgent) = length(a.horde)

function CritterbotTPCOriginalAuxTaskAgent(parsed; rng=Random.GLOBAL_RNG)
    # RNN architecture originally used, with RNN -> linear output
    nhidden = parsed["rnn_nhidden"]
    cell = getproperty(Flux, Symbol(parsed["rnn_cell"]))
    ...
    return _CritterbotTPCAuxTaskAgent(parsed, chain, horde; rng=rng)
end

function CritterbotTPCAuxTaskAgent(parsed; rng=Random.GLOBAL_RNG)
    # Uses an architecture more similar to the GVFN, with
    # a recurrent layer producing a representation, and
    # a FC NN producing timeseries predictions from this.
...
function _CritterbotTPCAuxTaskAgent(parsed, chain, horde; rng=Random.GLOBAL_RNG)
...
function resetBatch!(agent::CritterbotTPCAuxTaskAgent)
    # reset the agent's batch buffers
    agent.batch_obs, agent.batch_h, agent.batch_gvfn_target, agent.batch_model_target = getNewBatch()
end

function MinimalRLCore.start!(agent::CritterbotTPCAuxTaskAgent, env_s_tp1, rng=Random.GLOBAL_RNG)
...
function MinimalRLCore.step!(agent::CritterbotTPCAuxTaskAgent, env_s_tp1, r, terminal, rng=Random.GLOBAL_RNG)
...
function predict!(agent::CritterbotTPCAuxTaskAgent, env_s_tp1, r, terminal, rng=Random.GLOBAL_RNG)
    # for validation/test; predict, updating hidden states, but don't update models
